  ...
  const handleAuth = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        const newPumpRef = await addDoc(collection(db, `tenants/${tenantId}/pumps`), {
          name: pumpName.trim(),
          hardware: [{ id: 'du1', name: 'Dispensing Unit 1', nozzles: [{ id: 'ms', type: 'Petrol (MS)', color: 'orange' }, { id: 'hsd', type: 'Diesel (HSD)', color: 'blue' }] }],
          createdAt: Date.now()
        });

        // FIXED: Added pumpName here so it doesn't show "Connecting..."
        const newUserData = { 
    ...
    <div className="min-h-screen flex font-sans bg-white">
      <div className="hidden lg:flex w-1/2 bg-slate-900 relative flex-col justify-between p-12 overflow-hidden">
        ...
        <div className="relative z-10 flex items-center gap-3">
          <div className="w-12 h-12 bg-blue-600 rounded-xl flex items-center justify-center shadow-lg shadow-blue-600/30"><Fuel size={24} className="text-white" /></div>
          <div><p className="text-white font-black text-2xl tracking-tight">FuelDesk ERP</p><p className="text-blue-400 text-xs font-bold tracking-widest uppercase mt-0.5">by Ekta Management</p></div>
        </div>

        <div className="relative z-10">
          <h2 className="text-5xl font-black text-white leading-tight tracking-tight mb-6">The complete <br /><span className="text-blue-500">petrol pump</span> <br />management suite.</h2>
          <div className="grid grid-cols-2 gap-4 max-w-lg">
            {['Daily Sales & Shifts', 'Indent & Stock', 'Staff Attendance', 'Credit Ledger', 'Density Logs', 'Multi-Pump Ready'].map(feature => (
              <div key={feature} className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-xl p-3"><CheckCircle2 size={16} className="text-blue-400" /><span className="text-white/80 text-sm font-semibold">{feature}</span></div>
            ))}
          </div>
        </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12 overflow-y-auto">
        <div className="w-full max-w-md">
          <div className="lg:hidden flex items-center gap-3 mb-10"><div className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center shadow-md"><Fuel size={20} className="text-white" /></div><span className="text-2xl font-black text-slate-900 tracking-tight">FuelDesk ERP</span></div>
          <div className="mb-8"><h2 className="text-3xl font-black text-slate-900 tracking-tight">{isLogin ? 'Welcome back' : 'Get started'}</h2><p className="text-slate-500 mt-2 text-sm font-medium">{isLogin ? 'Sign in to your pump management dashboard' : 'Register your petrol pump — secure & scalable'}</p></div>

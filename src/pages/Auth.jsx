  ...
  <div className="space-y-1.5">
    ...
    <div className="relative flex items-center">
      ...
      <input 
        ...
        className="w-full pl-12 pr-14 py-3.5 bg-slate-50 border-2 border-slate-200 rounded-xl text-sm font-semibold text-slate-900 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-50 transition-all" 
      />
...
export const Auth = ({ onLogin }) => {
  ...
  const handlePhoneInput = (e) => { const val = e.target.value.replace(/[^0-9]/g, ''); if (val.length <= 10) setFormData(p => ({ ...p, phone: val })); };

  const handleAuth = async (e) => {
    e.preventDefault(); setError(''); setLoading(true);
    const { phone, password, confirmPass, ownerName, pumpName } = formData;

    if (!/^\d{10}$/.test(phone)) { setError('Valid 10-digit phone number is required.'); setLoading(false); return; }
    ...
    try {
      if (isLogin) {
        ...
        if (userDoc.exists()) onLogin(userDoc.data());
        else setError('User profile not found in database. Contact Admin.');
      } else {
        // SIGNUP
        ...
        // 2. Create Pump in ROOT collection (Fixed Location)
        const newPumpRef = await addDoc(collection(db, "pumps"), {
          tenantId: tenantId, // Linking to tenant
          name: pumpName.trim(),
        ...
        // 3. Create User
        const newUserData = { 
          uid, name: ownerName.trim(), phone, role: 'Admin', 
          tenantId, accessiblePumps: [newPumpRef.id], 
  ...
  return (
    <div className="min-h-screen flex font-sans bg-white">
      ...
      <div className="hidden lg:flex w-1/2 bg-slate-900 relative flex-col justify-between p-12 overflow-hidden">
        <div className="absolute top-[-10%] right-[-10%] w-[500px] h-[500px] rounded-full bg-blue-600/20 blur-3xl mix-blend-screen pointer-events-none" />
        <div className="absolute bottom-[-10%] left-[-10%] w-[400px] h-[400px] rounded-full bg-indigo-600/20 blur-3xl mix-blend-screen pointer-events-none" />
        ...
        <div className="relative z-10 flex items-center gap-3">
          ...
          <div>
            <p className="text-white font-black text-2xl tracking-tight">FuelDesk ERP</p>
            <p className="text-blue-400 text-xs font-bold tracking-widest uppercase mt-0.5">by Ekta Management</p>
          </div>
        </div>

        <div className="relative z-10">
          <h2 className="text-5xl font-black text-white leading-tight tracking-tight mb-6">
            The complete <br /><span className="text-blue-500">petrol pump</span> <br />management suite.
          </h2>
          <div className="grid grid-cols-2 gap-4 max-w-lg">
            {['Daily Sales & Shifts', 'Indent & Stock', 'Staff Attendance', 'Credit Ledger', 'Density Logs', 'Multi-Pump Ready'].map(feature => (
              <div key={feature} className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-xl p-3">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12 overflow-y-auto">
        <div className="w-full max-w-md">
          <div className="lg:hidden flex items-center gap-3 mb-10">
            ...
            <span className="text-2xl font-black text-slate-900 tracking-tight">FuelDesk ERP</span>
          </div>
          ...
          <div className="mb-8">
            ...
            <p className="text-slate-500 mt-2 text-sm font-medium">
              {isLogin ? 'Sign in to your pump management dashboard' : 'Register your petrol pump — secure & scalable'}
            </p>
          </div>

          {error && (
            <div className="flex items-center gap-3 p-4 bg-red-50 border border-red-200 rounded-xl text-red-700 text-sm font-bold mb-6">
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            {!isLogin && (
              <>
                <InputField label="Pump Name" icon={Building2} placeholder="e.g. Shree Ram Petroleum" onChange={set('pumpName')} />
                <InputField label="Owner Name" icon={User} placeholder="Your full name" onChange={set('ownerName')} />
              </>
            )}
            ...
            <div className="pt-4">
              <button type="submit" disabled={loading} className="w-full flex items-center justify-center gap-2 py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-black text-sm uppercase tracking-widest transition-colors shadow-lg shadow-blue-600/30 disabled:opacity-70 disabled:cursor-not-allowed">
                {loading ? 'Processing...' : (isLogin ? 'Secure Login' : 'Create Account')} {!loading && <ArrowRight size={18} />}
              </button>
            </div>
          </form>
          ...
          <p className="mt-8 text-center text-sm text-slate-500 font-medium">
            ...
            <button onClick={() => { setIsLogin(!isLogin); setError(''); }} className="text-blue-600 font-bold ml-2 hover:underline">
              {isLogin ? 'Register your pump' : 'Sign in'}</button>
          </p>
        </div>
      </div>
    </div>
